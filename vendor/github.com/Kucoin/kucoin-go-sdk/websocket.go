...
func (wc *WebSocketClient) subscribe() {
	...
	for {
		...
		default:
			...
			case ErrorMessage:
				wc.errors <- fmt.Errorf("Error message: %s", ToJsonString(m))
				return
			default:
				wc.messages <- m
			}
...
func (wc *WebSocketClient) keepHeartbeat() {
	...
	for {
		...
		case <-pt.C:
			...
			case pid := <-wc.pongs:
				if pid != p.Id {
					wc.errors <- fmt.Errorf("Invalid pong id %s, expect %s", pid, p.Id)
					return
				}
			case <-time.After(time.Duration(wc.server.PingTimeout) * time.Millisecond):
				wc.errors <- fmt.Errorf("Wait pong message timeout in %d ms", wc.server.PingTimeout)
				return
			}
		}
	}
...
func (wc *WebSocketClient) waitQuit() {
	...
	case <-wc.done:
	case sg := <-wc.signals:
		wc.errors <- fmt.Errorf("Quit due to a signal: %s", sg.String())
	}
}
