...
func (ar *ApiResponse) ReadData(v interface{}) error {
	...
	if !ar.ApiSuccessful() {
		m := fmt.Sprintf("[API]Failure: api code is NOT %s, %s %s with body=%s, respond code=%v message=\"%s\" data=%s",
			ApiSuccess,
			ar.response.request.Method,
			ar.response.request.RequestURI(),
			string(ar.response.request.Body),
	...
	if len(ar.RawData) == 0 {
		m := fmt.Sprintf("[API]Failure: try to read empty data, %s %s with body=%s, respond code=%v message=\"%s\" data=%s",
			ar.response.request.Method,
			ar.response.request.RequestURI(),
			string(ar.response.request.Body),
			ar.Code,
